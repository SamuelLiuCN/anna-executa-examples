    ...
    "tools": [
        {
            ...
            "parameters": [
                ...
                {"name": "model", "type": "string", "description": "Optional image-model hint (MCP modelPreferences), e.g. 'fal-ai/nano-banana-2'. Miss falls back to the user's preferred image model.", "required": False, "default": ""},
            ],
        },
        {
            "name": "poster_restyle",
            ...
            "parameters": [
                ...
                {"name": "model", "type": "string", "description": "Optional image-model hint (MCP modelPreferences).", "required": False, "default": ""},
            ],
        },
        {
            "name": "poster_persist",
...
async def _poster_create(topic: str, style: str = "modern minimalist", size: str = "1024x1024", model: str = "", *, invoke_id: str) -> dict:
    ...
    result = await image.generate(
        ...
        n=1,
        size=size,
        # Request-level model preference (MCP shape). The host resolves the
        # hint against active image models (plan-checked); a miss silently
        # falls back to the user's preferred image model — never an error.
        model_preferences=(
            {"hints": [{"name": model.strip()}]} if model and model.strip() else None
        ),
...
async def _poster_restyle(image_url: str, style: str, model: str = "", *, invoke_id: str) -> dict:
